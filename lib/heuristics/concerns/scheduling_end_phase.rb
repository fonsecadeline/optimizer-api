...
module SchedulingEndPhase
  ...
  def reaffect_prohibiting_partial_assignment(still_removed)
    ...
    banned = []
    adapted_still_removed = still_removed.collect(&:first).uniq.collect{ |id| [id, @services_data[id][:raw].visits_number] }
    # uniq_ids = still_removed.uniq(&:first)
    # adapted_still_removed = uniq_ids.collect{ |id| [id, @services_data[id][:raw].visits_number] }
    ...
    until most_prio_and_frequent.empty?
      ...
      if sequences.empty?
        ...
        @output_tool&.insert_visits(to_plan[:vehicle], @services_data[to_plan[:service]][:used_days], to_plan[:service], @services_data[to_plan[:service]][:raw].visits_number)
