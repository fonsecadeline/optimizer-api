...
module Interpreters
  class PeriodicVisits
    ...
    def possible_days_from_possible_day(service, possible_days, vehicles, lapse, options)
      day = @schedule_start
      nb_services_seen = 0
      set = []
      while day <= @schedule_end && nb_services_seen < service.visits_number
        ...
        day =
          if lapse
            [possible_days[nb_services_seen], day].compact.max
          else
            # case with maximum_lapse, but maximum_lapse is nil
            @schedule_end - (service.visits_number - nb_services_seen - 1) * (service.minimum_lapse || 1)
          end

        while (service.unavailable_days.include?(day) || vehicles.none?{ |v| v.available_at(day) } || # day is not available
              nb_services_seen > 0 && day <= set[nb_services_seen - 1] ||
              options[:first_possible_days][nb_services_seen] && options[:first_possible_days][nb_services_seen] > day) &&
    ...
    def compute_possible_days(vrp)
      ...
      vrp.services.each{ |service|
        service.first_possible_days =
          possible_days_from_possible_day(service, service[:first_possible_days], vrp.vehicles,
                                          service.minimum_lapse || 1, { is_min_lapse: true, first_possible_days: [] })
        if service.last_possible_days.any?
          service.last_possible_days =
            possible_days_from_possible_day(service, service[:last_possible_days], vrp.vehicles, service.maximum_lapse,
          ...
          while day >= @schedule_start && nb_services_seen < service.visits_number
            day -= 1 while (service.unavailable_days.include?(day) || vrp.vehicles.none?{ |v| v.available_at(day) } ||
                           service.last_possible_days[nb_services_seen - 1]) && day >= @schedule_start
