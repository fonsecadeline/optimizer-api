  ...
  module V01
    ...
    class Vrp < APIBase
      ...
      def self.vrp_request_resolution(this)
        ...
        this.optional(:minimize_used_days, type: Boolean, default: false, desc: '(Scheduling heuristic only) Tries to fill minimum possible days. Starts filling sooner days. Only available if first_solution_strategy is \'periodic\'. Not available ORtools.')
        this.optional(:same_point_day, type: Boolean, desc: '(Scheduling heuristic only) Forces all services with the same point_id to take place on the same days. Only available if first_solution_strategy is \'periodic\'. Not available ORtools.')
        this.optional(:allow_partial_assignment, type: Boolean, default: true, desc: '(Scheduling heuristic only) Assumes solution is valid even if only a subset of one service\'s visits are affected. Default: true. Not available ORtools.')
